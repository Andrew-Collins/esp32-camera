...
static int write_reg(uint8_t slv_addr, const uint16_t reg, uint8_t value) {
  ...
  if ((uint8_t)old_value != value) {
    ESP_LOGI(TAG, "NEW REG 0x%04x: 0x%02x to 0x%02x", reg, (uint8_t)old_value,
             value);
    ret = SCCB_Write16(slv_addr, reg, value);
  } else {
...
static int set_reg_bits(uint8_t slv_addr, uint16_t reg, uint8_t offset,
...
static int write_reg16(uint8_t slv_addr, const uint16_t reg, uint16_t value) {
  if (write_reg(slv_addr, reg, value >> 8) ||
      write_reg(slv_addr, reg + 1, value)) {
    return -1;
  }
  return 0;
}

static int write_addr_reg(uint8_t slv_addr, const uint16_t reg,
                          uint16_t x_value, uint16_t y_value) {
  if (write_reg16(slv_addr, reg, x_value) ||
      write_reg16(slv_addr, reg + 2, y_value)) {
    return -1;
  }
  return 0;
}

#define write_reg_bits(slv_addr, reg, mask, enable)                            \
  set_reg_bits(slv_addr, reg, 0, mask, (enable) ? (mask) : 0)
...
static int calc_sysclk(int xclk, bool pll_bypass, int pll_multiplier,
  ...
  unsigned int PLL_CLK =
      pll_bypass
          ? (xclk)
          : (VCO / pll_sys_div * 2 / 5); // 5 here is 10bit mode / 2, for 8bit
  ...
  ESP_LOGI(TAG,
           "Calculated XVCLK: %d Hz, REFIN: %u Hz, VCO: %u Hz, PLL_CLK: %u Hz, "
           "SYSCLK: %u Hz, PCLK: %u Hz",
           xclk, REFIN, VCO, PLL_CLK, SYSCLK, PCLK);
  return SYSCLK;
}

static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier,
                   uint8_t sys_div, uint8_t pre_div, bool root_2x,
  ...
  if (ret == 0) {
    ret = write_reg(sensor->slv_addr, 0x3037,
                    (pre_div & 0xf) | (root_2x ? 0x10 : 0x00));
  }
  if (ret == 0) {
    ret =
        write_reg(sensor->slv_addr, 0x3108, (pclk_root_div & 0x3) << 4 | 0x06);
  }
  if (ret == 0) {
  ...
  if (ret == 0) {
    ret = write_reg(sensor->slv_addr, 0x3103,
...
static int set_image_options(sensor_t *sensor) {
  ...
  ESP_LOGD(TAG,
           "Set Image Options: Compression: %u, Binning: %u, V-Flip: %u, "
           "H-Mirror: %u, Reg-4514: 0x%02x",
           sensor->pixformat == PIXFORMAT_JPEG, sensor->status.binning,
...
static int set_framesize(sensor_t *sensor, framesize_t framesize) {
  ...
  sensor->status.binning =
      (w <= (settings.max_width / 2) && h <= (settings.max_height / 2));
  sensor->status.scale =
      !((w == settings.max_width && h == settings.max_height) ||
        (w == (settings.max_width / 2) && h == (settings.max_height / 2)));

  ret = write_addr_reg(sensor->slv_addr, X_ADDR_ST_H, settings.start_x,
                       settings.start_y) ||
        write_addr_reg(sensor->slv_addr, X_ADDR_END_H, settings.end_x,
                       settings.end_y) ||
  ...
  if (!sensor->status.binning) {
    ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, settings.total_x,
                         settings.total_y) ||
          write_addr_reg(sensor->slv_addr, X_OFFSET_H, settings.offset_x,
                         settings.offset_y);
  } else {
    if (w > 920) {
      ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H,
                           settings.total_x - 200, settings.total_y / 2);
    } else {
      ret = write_addr_reg(sensor->slv_addr, X_TOTAL_SIZE_H, 2060,
                           settings.total_y / 2);
    }
    if (ret == 0) {
      ret = write_addr_reg(sensor->slv_addr, X_OFFSET_H, settings.offset_x / 2,
                           settings.offset_y / 2);
    }
  }

  if (ret == 0) {
    ret = write_reg_bits(sensor->slv_addr, ISP_CONTROL_01, 0x20,
                         sensor->status.scale);
  }
  ...
  if (sensor->pixformat == PIXFORMAT_JPEG) {
    ...
    // Set PLL: bypass: 0, multiplier: sys_mul, sys_div: 4, pre_div: 2, root_2x:
    // 0, pclk_root_div: 2, pclk_manual: 1, pclk_div: 4
  } else {
...
static int set_colorbar(sensor_t *sensor, int enable) {
  int ret = 0;
  ret = write_reg_bits(sensor->slv_addr, PRE_ISP_TEST_SETTING_1, TEST_COLOR_BAR,
                       enable);
  if (ret == 0) {
...
static int set_gain_ctrl(sensor_t *sensor, int enable) {
  int ret = 0;
  ret = write_reg_bits(sensor->slv_addr, AEC_PK_MANUAL,
...
static int set_agc_gain(sensor_t *sensor, int gain) {
  ...
  ret = write_reg(sensor->slv_addr, 0x350a, gainv >> 8) ||
        write_reg(sensor->slv_addr, 0x350b, gainv & 0xff);
  if (ret == 0) {
...
static int _set_pll(sensor_t *sensor, int bypass, int multiplier, int sys_div,
                    ...
                    int pclk_div) {
  int ret = 0;
  ret = set_pll(sensor, bypass > 0, multiplier, sys_div, pre_div, root_2x > 0,
                seld5, pclk_manual > 0, pclk_div);
  return ret;
}
...
static int disable_clks(sensor_t *sensor) {
  int ret = 0;
  ret += write_reg(sensor->slv_addr, 0x3004, 0);
  ret += write_reg(sensor->slv_addr, 0x3005, 0);
  ret += write_reg(sensor->slv_addr, 0x3006, 0);
  ret += write_reg(sensor->slv_addr, 0x3007, 0);
  return ret;
}

static int set_pwdn(sensor_t *sensor, bool state) {
  return write_reg(sensor->slv_addr, SYSTEM_CTROL0, 0x42);
}

static int set_flash(sensor_t *sensor, int type, bool state) {
  // Which frame after flash request is properly exposed
  uint8_t frame_wait[] = {3, 3, 3, 1};
  uint8_t st = state;
  uint8_t ty = type;
  // Only 4 valid types
  if (type > 3 || type < 0) {
  ...
  /* int ret = 0; */
  int ret =
      write_reg(sensor->slv_addr, STROBE_CTRL, (st << 7) | (0x00 << 2) | ty);
...
static int frex_req(sensor_t *sensor) {
  int ret = write_reg(sensor->slv_addr, 0x3B08, 1);
  // Send -1 if failed write, otherwise send number of frames to wait
  return ret;
}
...
static int init_status(sensor_t *sensor) {
  ...
  sensor->status.agc = !check_reg_mask(sensor->slv_addr, AEC_PK_MANUAL,
                                       AEC_PK_MANUAL_AGC_MANUALEN);
  sensor->status.aec = !check_reg_mask(sensor->slv_addr, AEC_PK_MANUAL,
                                       AEC_PK_MANUAL_AEC_MANUALEN);
  sensor->status.hmirror = check_reg_mask(sensor->slv_addr, TIMING_TC_REG21,
                                          TIMING_TC_REG21_HMIRROR);
  sensor->status.vflip =
      check_reg_mask(sensor->slv_addr, TIMING_TC_REG20, TIMING_TC_REG20_VFLIP);
  sensor->status.colorbar =
      check_reg_mask(sensor->slv_addr, PRE_ISP_TEST_SETTING_1, TEST_COLOR_BAR);
  ...
  sensor->status.quality =
      read_reg(sensor->slv_addr, COMPRESSION_CTRL07) & 0x3f;
...
int ov5640_init(sensor_t *sensor) {
  ...
  sensor->set_pwdn = set_pwdn;
  sensor->disable_clks = disable_clks;
  return 0;
}
