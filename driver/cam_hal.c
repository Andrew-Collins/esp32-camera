...
static const uint32_t JPEG_SOI_MARKER =
    0xFFD8FF; // written in little-endian for esp32
static const uint16_t JPEG_EOI_MARKER =
    0xD9FF; // written in little-endian for esp32
    ...
    case CAM_STATE_READ_BUF: {
      ...
      if (cam_event == CAM_IN_SUC_EOF_EVENT) {
        ...
        if (cam_obj->jpeg_mode && cnt == 0 &&
            cam_verify_jpeg_soi(frame_buffer_event->buf,
                                frame_buffer_event->len) != 0) {
          ll_cam_stop(cam_obj);
          cam_obj->state = CAM_STATE_IDLE;
        }
        cnt++;
        ...
        if (cnt || !cam_obj->jpeg_mode || cam_obj->psram_mode) {
          ...
          if (cam_obj->psram_mode) {
            ...
            if (frame_buffer_event->len != cam_obj->fb_size) {
              ...
              ESP_LOGE(TAG, "FB-SIZE: %u != %u", frame_buffer_event->len,
                       cam_obj->fb_size);
            }
          }
          // send frame
...
static esp_err_t cam_dma_config(const camera_config_t *config) {
  ...
  ESP_LOGI(TAG,
           ...
           cam_obj->dma_buffer_size, cam_obj->dma_half_buffer_size,
           cam_obj->dma_node_buffer_size, cam_obj->dma_node_cnt,
           cam_obj->frame_copy_cnt);

  cam_obj->dma_buffer = NULL;
  cam_obj->dma = NULL;

  cam_obj->frames = (cam_frame_t *)heap_caps_calloc(
      1, cam_obj->frame_cnt * sizeof(cam_frame_t), MALLOC_CAP_DEFAULT);
  ...
  for (int x = 0; x < cam_obj->frame_cnt; x++) {
    ...
    if (cam_obj->psram_mode) {
      ...
      ESP_LOGI(TAG, "Frame[%d]: Offset: %u, Addr: 0x%08X", x,
               cam_obj->frames[x].fb_offset,
               (uint32_t)cam_obj->frames[x].fb.buf);
  ...
  if (!cam_obj->psram_mode) {
    ...
    if (NULL == cam_obj->dma_buffer) {
      ESP_LOGE(TAG,
               ...
               "free block:%d Byte",
               __FUNCTION__, __LINE__, cam_obj->dma_buffer_size,
...
esp_err_t cam_config(const camera_config_t *config, framesize_t frame_size,
  ...
  cam_obj->event_queue =
      xQueueCreate(cam_obj->dma_half_buffer_cnt - 1, sizeof(cam_event_t));
  ...
  xTaskCreatePinnedToCore(cam_task, "cam_task", 2048, NULL,
                          configMAX_PRIORITIES - 2, &cam_obj->task_handle, 0);
#elif CONFIG_CAMERA_CORE1
  xTaskCreatePinnedToCore(cam_task, "cam_task", 2048, NULL,
                          configMAX_PRIORITIES - 2, &cam_obj->task_handle, 1);
#else
  xTaskCreate(cam_task, "cam_task", 2048, NULL, configMAX_PRIORITIES - 2,
              &cam_obj->task_handle);
#endif
