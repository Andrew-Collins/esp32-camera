...
static esp_err_t camera_probe(const camera_config_t *config,
  ...
  *out_camera_model = CAMERA_NONE;
  ...
  if (CAMERA_NONE == *out_camera_model) { // If no supported sensors are
                                          // detected
    CAMERA_DISABLE_OUT_CLOCK();
...
esp_err_t esp_camera_init(const camera_config_t *config) {
  ...
  if (PIXFORMAT_JPEG == pix_format &&
      (!camera_sensor[camera_model].support_jpeg)) {
  ...
  if (frame_size > camera_sensor[camera_model].max_size) {
    ESP_LOGW(TAG, "The frame size exceeds the maximum for this sensor, it will "
                  "be forced to the maximum possible value");
